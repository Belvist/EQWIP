  ...
  try {
    ...
    if (!userId) {
      if (process.env.TEST_MODE === '1') {
        const plan = [
          { skill: 'python', level: 'intermediate', target: 'advanced' },
          { skill: 'django', level: 'beginner', target: 'intermediate' },
        ]
        let ai: any = null
        try {
          if (process.env.AI_OFFLINE_ONLY !== '1') {
            const sys = 'You are a mentor. Produce a 4-week study plan. Output JSON {weeks: [{week, focus, resources[]}]} in Russian.'
            const prompt = `SKILL GAPS & PLAN INPUT:\n${JSON.stringify(plan)}\nReturn strictly JSON.`
            const out = await llmChat(prompt, { system: sys, model: process.env.GIGACHAT_CHAT_MODEL, json: true, timeoutMs: 10000 })
            const parsed = tryParseJson(out)
            if (parsed.ok) ai = parsed.data
          }
        } catch {}
        return NextResponse.json({ ok: true, data: plan, total: plan.length, ai, test: true })
      }
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
    }

    const engine = new CareerMapEngine()
    const plan = await engine.getSkillDevelopmentPlan(userId)
    try {
      if (process.env.AI_OFFLINE_ONLY === '1') {
      ...
      const parsed = tryParseJson(out)
      if (!parsed.ok || !parsed.data?.weeks) {
        return NextResponse.json({ error: 'LLM returned invalid JSON' }, { status: 502 })
      }
      return NextResponse.json({ ok: true, data: plan, total: plan.length, ai: parsed.data })
    } catch (e) {
      return NextResponse.json({ error: 'LLM unavailable' }, { status: 502 })
    }
  } catch (error) {
    console.error('Error getting skill development plan:', error)
