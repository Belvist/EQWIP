  ...
  const selectPlan = async (planId: string) => {
    ...
    try {
      const res = await fetch('/api/subscriptions', {
        ...
        body: JSON.stringify({ plan: planEnum, period: billingPeriod })
      })
      if (res.ok) {
        setCurrentPlan(planEnum)
        toast({ title: 'Тариф активирован', description: 'Вы успешно выбрали тариф' })
      } else {
        const err = await res.json().catch(() => ({}))
        toast({ title: 'Ошибка', description: err.message || 'Не удалось применить тариф', variant: 'destructive' })
      }
    } catch (e) {
